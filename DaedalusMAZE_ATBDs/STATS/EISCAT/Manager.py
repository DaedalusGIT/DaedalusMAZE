        ...
        for idx_lev in range(0, len(ALTs[0]), step):
            for idx_lat in range(0, len(ALTs[0,0]), step):
                for idx_lon in range(0, len(ALTs[0,0,0]), step):
                    ...
                    if "001" in CDF_filename:
                        if (idx_time==10): #if( random.randint(1,20) ) == 10:
                            print(curr_kp,curr_alt_km,curr_lat,curr_mlt, "   ", kp_to_fall,alt_to_fall,lat_to_fall,mlt_to_fall)
                    ...
                    if kp_to_fall is None or alt_to_fall is None or lat_to_fall is None or mlt_to_fall is None:
                        ...
                        #    print( "WARNING: [", curr_kp, curr_alt_km, curr_lat, curr_mlt, "] -> [", kp_to_fall, alt_to_fall, lat_to_fall, mlt_to_fall, "] not binned" )
                        break
                    else:
                        ...
                        ResultBuckets[ kp_to_fall, alt_to_fall, lat_to_fall, mlt_to_fall, "Vals" ].append( aValue )
            ...
            for aLat in Data.LATsequence:
                for aMLT in Data.MLTsequence:    
                    if len( ResultBuckets[ aKP, anALT, aLat, aMLT, "Vals" ] ) > 0:
                        ...
                        #    f.write( ' '.join([str(i) for i in ResultBuckets[aKP, anALT, aLat, aMLT, "Vals"]]) )
